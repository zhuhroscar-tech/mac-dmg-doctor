...
def parse_mounts(raw: str) -> Dict[str, str]:
    """Parse `mount` lines and return {mountpoint: device} for /Volumes paths."""
    mounts: Dict[str, str] = {}
...
def parse_hdiutil_info(raw: str) -> List[Dict[str, str]]:
    ...
    for line in raw.splitlines():
        ...
        if lower.startswith("/dev/") and "/" in line:
            # best-effort capture of hdiutil's top-level device token
...
def _recommendations(mount_point: str, device: str, image_path: str | None, holders: List[Dict[str, str]]) -> List[str]:
    recs: List[str] = []
    if holders:
        ...
        recs.append("Re-run `mac-dmg-doctor inspect <mountpoint>` after closing apps.")
    else:
...
def scan_busy_mounts() -> Dict[str, Any]:
    ...
    return {"mounted": len(mounts), "busy": busy}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Diagnose busy mount conditions on macOS")
...
def _print_scan(report: Dict[str, Any]) -> None:
    ...
    for item in report["busy"]:
        print(item["mountpoint"])  # noqa: T201
